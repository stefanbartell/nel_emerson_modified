...
As input, we need to provide the Knowledge base `dnp_kb.owl` and NER pipeline
...
```bash
prodigy entity_linker.manual emersons_annotated assets/emerson_input_text.txt temp/my_nlp/ kb/dnp_kb.owl assets/entities.csv -F scripts/el_recipe.py
```
